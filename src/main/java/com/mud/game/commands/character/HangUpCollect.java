...
public class HangUpCollect extends BaseCommand {
    ...
    * */
    ...
    public void execute() throws JSONException, JsonProcessingException {
        ...
        // 检查玩家有没有挖矿技能
        if(!GameCharacterManager.characterHasSkill(caller, "skill_zhishi_caiyao")){
            session.sendText(JsonResponse.JsonStringResponse(new MsgMessage(GameWords.NO_COLLECT_SKILL)));
        }else{
            Runnable runnable = HangUpManager.start(caller, PlayerCharacterState.STATE_COLLECT, session);
            if(runnable != null){
